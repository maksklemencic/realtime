...
export const Sidebar: React.FC = () => {
    ...
    return (
        <div className="md:h-full h-16 md:w-20 lg:w-52 w-full md:pt-6 pt-2 pb-4 px-4 flex flex-col justify-between md:shadow-lg md:border-t-0 border-t dark:md:border-r">
                        ...
                        return (
                            <Link href={`/groups/${group?.id}?show=groupPosts`} key={group?.id}>
                                <div className=" flex items-center justify-center lg:justify-start gap-2 w-full p-2 my-1 text-sm font-semibold hover:bg-muted  rounded-md" >
                                    ...
                                    {group?.image !== null && !colors.includes(group.image) && (
                                        <img className='h-8 w-8 rounded-lg' src={group?.image} alt={group?.name} />
                                    )}
